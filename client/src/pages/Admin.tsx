  ...
  return (
    <div className="min-h-screen bg-[#030308] text-white">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div id="setup" className="mt-12 p-6 rounded-2xl bg-white/3 border border-white/10">
          <h2 className="text-pink-300 font-serif font-bold text-lg mb-4 flex items-center gap-2">
            <Database className="w-5 h-5" /> Cómo configurar Supabase
          </h2>
          <ol className="space-y-4 text-sm text-white/70">
            <li className="flex gap-3">
              <span className="w-6 h-6 rounded-full bg-pink-500/20 text-pink-300 flex items-center justify-center text-xs font-bold shrink-0">1</span>
              <span>Creá una cuenta gratis en <a href="https://supabase.com" target="_blank" rel="noreferrer" className="text-pink-400 underline">supabase.com</a> y creá un nuevo proyecto.</span>
            </li>
            <li className="flex gap-3">
              <span className="w-6 h-6 rounded-full bg-pink-500/20 text-pink-300 flex items-center justify-center text-xs font-bold shrink-0">2</span>
              <div>
                Abrí el <strong className="text-white">SQL Editor</strong> en Supabase y ejecutá:
                <pre className="mt-2 p-3 bg-black/40 rounded-xl text-xs text-green-300 overflow-x-auto whitespace-pre-wrap">{`create table public.galaxy_photos (
...
create policy "read" on public.galaxy_photos for select using (true);
create policy "insert" on public.galaxy_photos for insert with check (true);
create policy "delete" on public.galaxy_photos for delete using (true);`}</pre>
              </div>
            </li>
            <li className="flex gap-3">
              <span className="w-6 h-6 rounded-full bg-pink-500/20 text-pink-300 flex items-center justify-center text-xs font-bold shrink-0">3</span>
              <div>
                En <strong className="text-white">Storage</strong>, creá un bucket llamado <code className="bg-white/10 px-1.5 rounded text-pink-300">galaxy-photos</code> y marcalo como <strong className="text-white">público</strong>.
              </div>
            </li>
            <li className="flex gap-3">
              <span className="w-6 h-6 rounded-full bg-pink-500/20 text-pink-300 flex items-center justify-center text-xs font-bold shrink-0">4</span>
              <div>
                En Supabase → <strong className="text-white">Project Settings → API</strong>, copiá la URL y la anon key. Agregá estas variables en <strong className="text-white">Netlify → Site Settings → Environment Variables</strong>:
                <pre className="mt-2 p-3 bg-black/40 rounded-xl text-xs text-yellow-300">{`VITE_SUPABASE_URL=https://xxxxxx.supabase.co
VITE_SUPABASE_ANON_KEY=eyJhbGci...`}</pre>
              </div>
            </li>
            <li className="flex gap-3">
              <span className="w-6 h-6 rounded-full bg-pink-500/20 text-pink-300 flex items-center justify-center text-xs font-bold shrink-0">5</span>
              <span>Hacé un nuevo deploy en Netlify (o pusheá a Git). ¡Listo! Ahora podés subir fotos desde el celu.</span>
            </li>
          </ol>
        </div>
      </div>
